...
export const AGENTS: AgentDef[] = [
  ...
  },
  {
    slug: "contract-diff",
    label: "판본 비교",
    blurb: "상대가 고쳐 보낸 초안을 이전 판과 대조해 무엇이 바뀌고 무엇이 빠졌는지 가립니다.",
    ...SALES,
    stage: "계약",
    // 검토·대책·작성이 모두 한 판본만 다룬다. 협상은 3~4 라운드를 도는데, 라운드마다
    // "우리가 요청한 것이 반영됐나"와 "합의한 것이 되돌아가지 않았나"를 사람 눈으로
    // 따라가는 것은 실제로 불가능하고, 사고는 늘 거기서 난다.
    instructions:
      "`contract-diff` 스킬을 읽고 그 규칙에 따라 답한다. 이전 문안과 새 문안을 " +
      "**둘 다** 인용하고 각 인용에 어느 파일에서 왔는지 파일명을 붙인다. 변경마다 " +
      "유리·불리·중립을 판정한다. 되돌아간 조항은 없으면 '없음'이라고 명시한다 — " +
      "비어 있는 것과 확인하지 않은 것은 다르다. 우리 요청 목록이 입력에 없으면 " +
      "대조표를 지어내지 말고 `요청 목록 미제공`이라 쓴다.",
    corpus: true,
    playbooks: ["contract-diff"],
    starters: [
      "첨부한 두 판본을 비교해서 뭐가 바뀌었는지 정리해 줘.",
      "우리가 요청한 수정이 이번 판에 반영됐는지 대조해 줘.",
      "합의했던 조항이 되돌아간 게 있는지 확인해 줘.",
    ],
  },
  {
    slug: "contract-legal",
    label: "법무 검토 요청",
    blurb: "검토에서 남은 법적 쟁점을 법무가 바로 답할 수 있는 질의서로 만듭니다.",
    ...SALES,
    stage: "계약",
    // 검토 결과는 매번 "법무 확인 필요"로 끝나는데 그 항목을 담을 그릇이 없었다.
    // 검토서를 통째로 법무에 넘기면 느리고 일반적인 답이 온다 — 독자가 다른
    // 문서이기 때문이다.
    instructions:
      "`contract-legal-brief` 스킬을 읽고 그 규칙에 따라 답한다. 항목마다 조항 " +
      "원문을 그대로 인용하고, 우리 입장·질문·필요한 결정과 기한을 함께 쓴다. " +
      "법적 결론을 대신 내리지 않는다 — 이 문서는 질문지다. **법조문 번호를 쓰지 " +
      "말고 쟁점의 이름으로 쓴다** — 틀린 조문 번호는 질의서 전체의 신뢰를 깎고, " +
      "계약서에 없는 숫자라 대조로 잡히지도 않는다. 질의 항목은 다섯 개를 " +
      "넘기지 않는다. 금액·기한을 지어내지 말고 모르면 `미확인`으로 남긴다. " +
      "담당자 이름과 연락처는 넣지 않는다.",
    playbooks: ["contract-legal-brief"],
    starters: [
      "이 검토 결과에서 법무에 물어야 할 것만 뽑아 질의서로 만들어 줘.",
      "무제한 배상 조항의 유효성을 법무에 묻는 요청서를 써 줘.",
      "해외 고객 계약이라 준거법·관할을 법무에 확인하고 싶어.",
    ],
  },
  {
    slug: "contract-ops",

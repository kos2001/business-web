...
측정해 보면 길이에 따라 갈립니다. 같은 계약서 전면 검토 프롬프트로 두 모델을 돌린
결과입니다:

| 모델 | 짧은 생성 | 긴 생성 (전면 검토) |
|---|---|---|
| `deepseek/deepseek-v4-flash-0731` | 4/4 정상 | **2/2 손상** |
| `z-ai/glm-5.3-flash` | — | 4/4 정상 (3,132~15,074토큰) |

프롬프트로 고칠 수 있는 문제가 아닙니다. SOUL 의 한자 금지 규칙은 이미 있었고,
디코더 이탈은 지시를 따르고 말고의 영역이 아닙니다.

## 모델을 바꾸고도 검수를 남긴 이유

모델 교체가 디코더 이탈은 없앴지만 **내용 오류는 없애지 못했습니다.** 손상 없이
통과한 glm-5.3-flash 답변 세 건을 검수기에 넣어 보면 세 건 모두에서 실제 오류가
나옵니다:

- 조항 오진술 — 원문은 `7일 전 통지로 해지`인데 본문은 `사유 불문 즉시 해지`
- `142,00×8,000=113,600,000` — 곱셈은 맞는데 단가 `14,200`의 자릿수가 깨짐
- `계약 위험` (→ 계약 위반), `배상율` (→ 배상률), `무상한 일 2.5% 배상`
- 을에게 불리한 배경 IP 양도 조항을 "관철해야 할 지점"이라고 뒤집어 서술

마지막에서 두 번째가 이 구조의 요점입니다. `142,00` 은 반복 루프도 아니고 한자도
아니라 기계 검사에 안 걸리고, 인용문 안이 아니고 뒤에 단위도 없어 수치 대조에도
안 걸립니다. **세 층 중 검수 에이전트만 잡습니다.** 계약서의 단가가 틀린 채로
협상 자료에 올라가면 안 되므로, 어떤 모델을 쓰든 검수는 남습니다.
